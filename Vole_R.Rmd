...
res2.A2 <-rcorr(as.matrix(BehDat[,4:26]),type = "pearson")
...
corrplot(res2.A2$r, type="upper", order="alphabet", 
...
BehDat.cntrl <- BehDat.cntrl[,4:11] %>% mutate_if(is.numeric, scale)
...
newdat <- cbind(BehDat.cntrl[,1:3],FS)

fit <- lm(cbind(ANX,SOC)~TRT*SEX, data=newdat)
summary(fit)
